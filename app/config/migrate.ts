...
// Migrate Nimbus3 config to Nimbus4 but only if the user hasn't manually
// touched the new config and if the old config is not a symlink
export const migrateNimbus3Config = () => {
  ...
  // Migrate
  copySync(resolve(dirname(legacyCfgPath), '.nimbus_plugins', 'local'), plugs.local);
  ...
  try {
    ...
    if (pluginCode) {
      const pluginPath = resolve(plugs.local, 'migrated-nimbus3-config.js');
      newCfgData.localPlugins = ['migrated-nimbus3-config', ...(newCfgData.localPlugins || [])];
  ...
  notify('Nimbus 4', `Settings location and format has changed to ${cfgPath}`);
};
