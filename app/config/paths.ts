...
let cfgDir = process.env.XDG_CONFIG_HOME
  ? join(process.env.XDG_CONFIG_HOME, 'Nimbus')
  : process.platform === 'win32'
    ? app.getPath('userData')
    : join(homeDirectory, '.config', 'Nimbus');
...
const cliScriptPath = resolve(__dirname, '../../bin/nimbus');
const cliLinkPath = '/usr/local/bin/nimbus';
