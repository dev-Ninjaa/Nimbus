...
// Handle EPIPE errors globally to prevent app crashes
process.on('uncaughtException', (err: any) => {
  if (err.code === 'EPIPE' || err.code === 'ERR_STREAM_DESTROYED' || err.code === 'ERR_IPC_CHANNEL_CLOSED') {
    console.warn('Caught EPIPE or stream error, ignoring:', err.message);
    return;
  }
    ...
    .then(() => {
      ...
      if (!isDev) {
        ...
        if (config.getConfig().defaultSSHApp && !app.isDefaultProtocolClient('ssh')) {
          console.log('Setting Nimbus as default client for ssh:// protocol');
          app.setAsDefaultProtocolClient('ssh');
        } else if (!config.getConfig().defaultSSHApp && app.isDefaultProtocolClient('ssh')) {
          console.log('Removing Nimbus from default client for ssh:// protocol');
