...
export function newWindow(
  ...
  const winOpts: BrowserWindowConstructorOptions = {
    ...
    title: 'Nimbus.app',
    // Use macOS-style transparent frameless window on all platforms
    frame: false,
    transparent: true,
    icon,
    show: true,
  ...
  // DevTools can be toggled with F12 or View > Toggle DevTools
  // No longer auto-opening in dev mode to improve startup performance
  ...
  rpc.on('new', (extraOptions) => {
    ...
    session.on('data', (data: string) => {
      try {
        if (!window.isDestroyed() && !rpc.win.isDestroyed()) {
    ...
    session.on('exit', () => {
      try {
        ...
        console.error('Error handling session exit:', err);
      }
    });
  });
  ...
  // Clean up sessions before window closes to prevent EPIPE errors
  window.on('close', () => {
    try {
      deleteSessions();
    } catch (err: any) {
      console.error('Error cleaning up sessions on window close:', err);
    }
  });
