...
const addSymlink = async (silent: boolean) => {
  try {
    ...
    if (isInstalled) {
      console.log('Nimbus CLI already in PATH');
      return;
    }
    console.log('Linking NimbusCLI');
    ...
    if (err.code === 'EACCES' && !silent) {
      const result = await dialog.showMessageBox({
        message: `You need to grant elevated privileges to add Nimbus CLI to PATH
Or you can run
sudo ln -sf "${cliScriptPath}" "${cliLinkPath}"`,
      ...
      if (result.response === 0) {
        try {
          await sudoExec(`ln -sf "${cliScriptPath}" "${cliLinkPath}"`, {name: 'Nimbus'});
          return;
        } catch (_error) {
...
const addBinToUserPath = () => {
  return new Promise<void>((resolve, reject) => {
    try {
      ...
      const oldPath = path.resolve(process.env.LOCALAPPDATA!, 'nimbus');
      ...
      console.log('Adding NimbusCLI path (registry)');
...
export const installCLI = async (withNotification: boolean) => {
  if (process.platform === 'win32') {
    try {
      ...
      logNotify(
        withNotification,
        'Nimbus CLI installation failed',
        `Failed to add Nimbus CLI path to user PATH ${err}`
      );
    }
    ...
    try {
      ...
      logNotify(withNotification, 'Nimbus CLI installed', `Symlink created at ${cliLinkPath}`);
    } catch (error) {
      logNotify(withNotification, 'Nimbus CLI installation failed', `${error}`);
    }
  } else {
    logNotify(withNotification, 'Nimbus CLI installation failed', `Unsupported platform ${process.platform}`);
  }
};
