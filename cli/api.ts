...
const applicationDirectory = process.env.XDG_CONFIG_HOME
  ? path.join(process.env.XDG_CONFIG_HOME, 'Nimbus')
  : process.platform === 'win32'
    ? path.join(process.env.APPDATA!, 'Nimbus')
    : path.join(os.homedir(), '.config', 'Nimbus');

const devConfigFileName = path.join(__dirname, `../nimbus.json`);

const fileName =
  process.env.NODE_ENV !== 'production' && fs.existsSync(devConfigFileName)
    ? devConfigFileName
    : path.join(applicationDirectory, 'nimbus.json');

/**
 * We need to make sure the file reading and parsing is lazy so that failure to
 * statically analyze the nimbus configuration isn't fatal for all kinds of
