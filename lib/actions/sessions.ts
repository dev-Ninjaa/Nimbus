...
import type {NimbusState, NimbusDispatch, NimbusActions} from '../../typings/nimbus';
import rpc from '../rpc';
import {keys} from '../utils/object';
import findBySession from '../utils/term-groups';

export function addSession({uid, shell, pid, cols = null, rows = null, splitDirection, activeUid, profile}: Session) {
  return (dispatch: NimbusDispatch, getState: () => NimbusState) => {
...
function createExitAction(type: typeof SESSION_USER_EXIT | typeof SESSION_PTY_EXIT) {
  return (uid: string) => (dispatch: NimbusDispatch, getState: () => NimbusState) => {
    return dispatch({
      type,
      uid,
      effect() {
...
export function clearActiveSession(): NimbusActions {
  return {
    type: SESSION_CLEAR_ACTIVE
  };
}

export function setSessionXtermTitle(uid: string, title: string): NimbusActions {
  return {
