...
import type {ITermState, ITermGroup, NimbusState, NimbusDispatch, NimbusActions} from '../../typings/nimbus';
import rpc from '../rpc';
...
function requestSplit(direction: 'VERTICAL' | 'HORIZONTAL') {
  return (_activeUid: string | undefined, _profile: string | undefined) =>
    (dispatch: NimbusDispatch, getState: () => NimbusState): void => {
      dispatch({
...
export function resizeTermGroup(uid: string, sizes: number[]): NimbusActions {
  return {
    uid,
