...
import type {NimbusState, NimbusProps, NimbusDispatch} from '../../typings/nimbus';
...
const Nimbus = forwardRef<HTMLDivElement, NimbusProps>((props, ref) => {
  ...
  const {customCSS, uiFontFamily, borderColor, fullScreen} = props;
  // Always use no border for premium macOS-style look
  const borderWidth = '';
  stylis.set({prefix: false});
  return (
    <div id="nimbus" ref={ref}>
      <div
        style={{fontFamily: uiFontFamily, borderColor, borderWidth}}
        className={`nimbus_main nimbus_mainRounded ${fullScreen ? 'fullScreen' : ''}`}
      >
        <HeaderContainer />
      ...
      {/*
        Add custom CSS to Nimbus.
        We add a scope to the customCSS so that it can get around the weighting applied by styled-jsx
      */}
      <style dangerouslySetInnerHTML={{__html: stylis('#nimbus', customCSS)}} />
    </div>
  );
});

Nimbus.displayName = 'Nimbus';
...
const mapDispatchToProps = (dispatch: NimbusDispatch) => {
  return {
    execCommand: (command: string, fn: (e: any, dispatch: NimbusDispatch) => void, e: any) => {
      dispatch(uiActions.execCommand(command, fn, e));
    }
  };
};

const NimbusContainer = connect(mapStateToProps, mapDispatchToProps, null, {forwardRef: true})(Nimbus, 'Nimbus');

export default NimbusContainer;

export type NimbusConnectedProps = ReturnType<typeof mapStateToProps> & ReturnType<typeof mapDispatchToProps>;
