...
Module._load = function _load(path: string) {
  ...
  switch (path) {
    ...
    case 'nimbus/component':
      console.warn(
        'DEPRECATED: If your plugin requires `nimbus/component`, it must requires `react.PureComponent` instead and bundle `react` as a dependency'
      );
...
const loadModules = () => {
  ...
  modules = paths.plugins
    ...
    .map((path) => {
      ...
      // mapNimbusTermState mapping for backwards compatibility with Nimbusterm
      if (mod.mapNimbusTermState) {
        mod.mapNimbusState = mod.mapNimbusTermState;
        console.error('mapNimbusTermState is deprecated. Use mapNimbusState instead.');
      }

      // mapNimbusTermDispatch mapping for backwards compatibility with Nimbusterm
      if (mod.mapNimbusTermDispatch) {
        mod.mapNimbusDispatch = mod.mapNimbusTermDispatch;
        console.error('mapNimbusTermDispatch is deprecated. Use mapNimbusDispatch instead.');
      }
      ...
      if (mod.mapNimbusState) {
        connectors.Nimbus.state.push(mod.mapNimbusState);
      }

      if (mod.mapNimbusDispatch) {
        connectors.Nimbus.dispatch.push(mod.mapNimbusDispatch);
      }
    ...
    .filter((mod): mod is NimbusPlugin => Boolean(mod));
